...
LocalAccessible* RelatedAccIterator::Next() {
  ...
  if (mIter == mRelatedNodes->end()) {
    return nullptr;
  }

  LocalAccessible* next = nullptr;
  do {
    next = mDocument->GetAccessible(*mIter);
    if (!next && *mIter == mDocument->GetContent()) {
      next = mDocument;
    }
  } while (++mIter < mRelatedNodes->end() && !next);
